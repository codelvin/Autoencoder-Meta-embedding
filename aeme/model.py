...
class AbsModel(object):
    ...
    @staticmethod
    def mae(x,y,f):
        """just like before mae with slicing"""
        x_d = x.get_shape().as_list()[1]
        y_d = y.get_shape().as_list()[1]
        if x_d != y_d:
            smaller = min(x_d, y_d)
            x = tf.slice(x, [0, 0], [tf.shape(x)[0], smaller])
            y = tf.slice(y, [0, 0], [tf.shape(y)[0], smaller])
        return tf.scalar_mul(f, tf.reduce_mean(tf.abs(tf.subtract(x, y))))
    ...
    def scp(x, y, f):
        ...
        if x_d != y_d:
            ...
            x = tf.slice(x, [0, 0], [tf.shape(x)[0], smaller])
            y = tf.slice(y, [0, 0], [tf.shape(y)[0], smaller])
        tmp = 1- tf.divide(tf.reduce_sum(tf.multiply(x, y)), tf.norm(x)*tf.norm(y)) #dot product / l2 norm
        return tf.scalar_mul(f, tmp)
    ...
    def kl(x, y, f):
        ...
        if x_d != y_d:
            ...
            x = tf.slice(x, [0, 0], [tf.shape(x)[0], smaller])
            y = tf.slice(y, [0, 0], [tf.shape(y)[0], smaller])
        episilon = tf.constant(1e-6, dtype =tf.float32, shape=[1, min(x_d, y_d)])
        return tf.scalar_mul(f, tf.reduce_sum(tf.multiply(x, tf.math.log(x + episilon)- tf.math.log(y + episilon))))
...
class DAEME(AbsModel):
    ...
    def build(self, srcs, ipts):
        ...
        self.encoders = [tf.layers.dense(ipt, 200, self.activ) for ipt, dim in zip(self.ipts, self.dims)]
